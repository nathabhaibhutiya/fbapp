        ...
        try{
		  ...
		  if(isset($this->access_token)){
            ...
            return $this->displayPhotos($_GET['id'],$_GET['title']);
			  ...
			  $loginUrl = $this->helper->getLoginUrl('https://sanjayfb.herokuapp.com/', $this->permissions);
   ...
   // print_r($_SESSION['facebook_access_token']);
    ...
    private function getData($album_id='',$type=''){
        if($type == 'photos'){
            ...
            $url = 'https://graph.facebook.com/v2.8/me/albums?access_token='.$this->access_token.'&limit=100&fields=id,name,cover_photo,count';
...
//print_r($url);
    ...
    public function test($file_url){
       // print_r($file_url);
        //$test_file = file_get_contents($file_url);
        //file_put_contents("test.jpg", file_get_contents($file_url));
